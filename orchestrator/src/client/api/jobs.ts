...
export async function getMarketStats(): Promise<MarketStats> {
	return fetchApi<MarketStats>("/jobs/market-stats");
}
...
export async function getSkillGapStats(minScore = 60): Promise<SkillGapStats> {
	return fetchApi<SkillGapStats>(`/jobs/skill-gap?minScore=${minScore}`);
}
...
export async function getEmployerInsights(): Promise<EmployerInsights> {
	return fetchApi<EmployerInsights>("/jobs/employer-insights");
}

export async function skipJobWithReason(
	id: string,
	reason: string,
): Promise<void> {
	await fetchApi<unknown>(`/jobs/${id}/skip`, {
		method: "POST",
...
export async function getJobsCompare(ids: string[]): Promise<CompareResponse> {
	return fetchApi<CompareResponse>(`/jobs/compare?ids=${ids.join(",")}`);
}
...
export async function getSkillTrends(
	weeks = 8,
): Promise<{ trends: SkillTrend[]; weeks: number }> {
	return fetchApi<{ trends: SkillTrend[]; weeks: number }>(
		`/jobs/skill-trends?weeks=${weeks}`,
	);
}
...
export async function getJobsBySkill(
	skill: string,
	minScore = 60,
): Promise<{ jobs: JobsBySkillEntry[] }> {
	return fetchApi<{ jobs: JobsBySkillEntry[] }>(
		`/jobs/by-skill?skill=${encodeURIComponent(skill)}&minScore=${minScore}`,
	);
}
...
export async function getPivotClusters(
	skills: string[],
): Promise<{ clusters: PivotCluster[]; userSkillCount: number }> {
	const q = `skills=${encodeURIComponent(skills.join(","))}`;
	return fetchApi<{ clusters: PivotCluster[]; userSkillCount: number }>(
		`/jobs/pivot-clusters?${q}`,
	);
}
...
export async function getSkillCooccurrence(
	...
	minScore = 60,
	limit = 10,
): Promise<{ skill: string; results: SkillCooccurrenceEntry[] }> {
	const params = new URLSearchParams({
		skill,
	...
	return fetchApi<{ skill: string; results: SkillCooccurrenceEntry[] }>(
		`/jobs/skill-cooccurrence?${params.toString()}`,
	);
}
...
export async function getSkillDemand(
	skills: string[],
	minScore = 0,
): Promise<{ results: SkillDemandEntry[]; skills: string[] }> {
	const q = `skills=${encodeURIComponent(skills.join(","))}&minScore=${minScore}`;
	return fetchApi<{ results: SkillDemandEntry[]; skills: string[] }>(
		`/jobs/skill-demand?${q}`,
	);
}
...
export async function getMarketTrend(weeks = 8): Promise<MarketTrendResult> {
	return fetchApi<MarketTrendResult>(`/jobs/market-trend?weeks=${weeks}`);
}
