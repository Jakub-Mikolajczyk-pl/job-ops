...
import { buildPrompt, EXTRACTION_SCHEMA } from "./recruitment-intake";
...
describe.sequential("Recruitment intake worker (R2)", () => {
  ...
  it("keeps the LLM schema compatible with OpenAI strict structured outputs", () => {
    const schema = EXTRACTION_SCHEMA.schema;
    const properties = Object.keys(schema.properties);

    expect(schema.additionalProperties).toBe(false);
    expect(EXTRACTION_SCHEMA).not.toHaveProperty("additionalProperties");
    ...
    for (const key of properties) {
      ...
      const isCoreRequiredField = [
        "isRecruitment",
        "company",
        "position",
        "stage",
      ].includes(key);
      if (!isCoreRequiredField) {
        expect(Array.isArray(property.type)).toBe(true);
  ...
  it("lists the exact extraction keys in the prompt for json_object fallback", () => {
    const prompt = buildPrompt("note", "Acme is hiring a Senior Java dev.");

    expect(prompt).toContain("Return one JSON object with exactly these keys");
    expect(prompt).toContain("company");
    expect(prompt).toContain("position");
    expect(prompt).toContain('"company": legal/company name');
    expect(prompt).toContain('"position": role/title');
  });
