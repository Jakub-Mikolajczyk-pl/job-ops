...
export async function generateInterviewPrep(
	...
	const summaryText = job.tailoredSummary
		? `Your tailored summary: ${job.tailoredSummary}`
		: "";
	const descSnippet = job.jobDescription
...
For each question, provide a short hint (1-2 sentences) on what a strong answer covers. Return JSON matching the schema.`;
	...
	if (!result.success) {
		logger.warn("Interview prep LLM call failed", {
			error: result.error,
			jobId: job.id,
		});
