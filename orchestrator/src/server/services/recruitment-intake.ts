...
function priorityToScore(p?: string): { score: number; priority: string } {
	const priority = (STUDY_TOPIC_PRIORITIES as readonly string[]).includes(
		p ?? "",
	)
		? (p as StudyTopicPriority)
		: "medium";
	const score = priority === "high" ? 85 : priority === "low" ? 35 : 60;
...
export async function processIntake(intakeId: string): Promise<IntakeResult> {
	...
	try {
		...
		if (!company || !position) {
			...
			return { intakeId, action: "needs_review", reason: "missing company/position" };
		}

		const jobUrl = `recruitment://${slug(company)}-${slug(position)}`;
		const { score, priority } = priorityToScore(ex.priority);
		...
		if (existing) {
			await updateJob(existing.id, {
				...
				suitabilityReason: clean(ex.flags),
			});
			await createJobNote({
				jobId: existing.id,
