...
export const jobs = sqliteTable(
	"jobs",
	{
		...
		// Canonicalized location and normalized salary (A2/A3)
		locationCity: text("location_city"),
		locationCountry: text("location_country"),
		monthlyMinPLN: real("monthly_min_pln"),
		monthlyMaxPLN: real("monthly_max_pln"),
		...
		// OE toolkit enrichments
		oeFitnessScore: real("oe_fitness_score"),
		oeFitnessReasons: text("oe_fitness_reasons"),
		redFlags: text("red_flags"),
		asyncScore: real("async_score"),
		asyncSignals: text("async_signals"),
		weeklyHoursEstimate: integer("weekly_hours_estimate"),
		weeklyHoursReasons: text("weekly_hours_reasons"),
...
export const savedSearches = sqliteTable("saved_searches", {
	...
	name: text("name").notNull(),
	query: text("query").notNull(), // JSON: { status, source, minScore, keywords, ... }
	notifyTelegram: integer("notify_telegram", { mode: "boolean" }).default(
		false,
	),
...
export type NewSkillSnapshotRow = typeof skillSnapshots.$inferInsert;

export const activeEmployments = sqliteTable(
	"active_employments",
	{
		id: text("id").primaryKey(),
		tenantId: text("tenant_id")
		...
		monthlyGrossPLN: real("monthly_gross_pln"),
		hourlyRatePLN: real("hourly_rate_pln"),
	...
	(table) => ({
		tenantIndex: index("idx_active_employments_tenant").on(table.tenantId),
	}),
);

export type ActiveEmploymentRow = typeof activeEmployments.$inferSelect;
export type NewActiveEmploymentRow = typeof activeEmployments.$inferInsert;

export const skillExclusions = sqliteTable(
	"skill_exclusions",
	{
		tenantId: text("tenant_id").notNull(),
		skill: text("skill").notNull(),
		excludedAt: text("excluded_at").notNull().default(sql`(datetime('now'))`),
		reason: text("reason"),
	},
	(table) => ({
		pk: uniqueIndex("skill_exclusions_pk").on(table.tenantId, table.skill),
	}),
);

export type SkillExclusionRow = typeof skillExclusions.$inferSelect;
export type NewSkillExclusionRow = typeof skillExclusions.$inferInsert;

export const skillWatchlist = sqliteTable(
	"skill_watchlist",
	{
		id: text("id").primaryKey(),
		tenantId: text("tenant_id").notNull(),
		...
		alertOnDrop: integer("alert_on_drop", { mode: "boolean" }).default(false),
		alertOnRise: integer("alert_on_rise", { mode: "boolean" }).default(false),
		dropThresholdPct: integer("drop_threshold_pct").default(30),
		riseThresholdPct: integer("rise_threshold_pct").default(30),
		notes: text("notes"),
		pendingMatchCount: integer("pending_match_count").default(0),
		lastMatchAt: text("last_match_at"),
		createdAt: text("created_at").notNull().default(sql`(datetime('now'))`),
	},
	(table) => ({
		tenantIdx: index("idx_skill_watchlist_tenant").on(table.tenantId),
		uniqueSkill: uniqueIndex("idx_skill_watchlist_skill_unique").on(
			table.tenantId,
			table.skill,
		),
	}),
...
export type SkillWatchlistRow = typeof skillWatchlist.$inferSelect;
export type NewSkillWatchlistRow = typeof skillWatchlist.$inferInsert;

export const profileMarketSnapshots = sqliteTable(
	"profile_market_snapshots",
	{
		tenantId: text("tenant_id").notNull(),
		snapshotDate: text("snapshot_date").notNull(),
		totalJobs: integer("total_jobs").notNull(),
		jobsAbove60: integer("jobs_above_60").notNull(),
		jobsAbove75: integer("jobs_above_75").notNull(),
		avgScore: real("avg_score"),
		capturedAt: text("captured_at").notNull().default(sql`(datetime('now'))`),
	},
	(table) => ({
		pk: uniqueIndex("profile_market_snapshots_pk").on(
			table.tenantId,
			table.snapshotDate,
		),
		tenantDateIdx: index("idx_profile_market_snapshots_tenant_date").on(
			table.tenantId,
			table.snapshotDate,
		),
	}),
...
export type ProfileMarketSnapshotRow =
