...
import { checkWatchlistMatches } from "../repositories/skill-watchlist";
import { computeAsyncScore } from "../services/oe-async-score";
import { computeOeFitness } from "../services/oe-fitness";
import { scan as scanRedFlags } from "../services/oe-redflags";
import { computeWorkloadEstimate } from "../services/oe-workload";
...
export async function runPipeline(
	...
	// A1: Filter out interactive-challenge extractors when running headless
	if (process.env.PIPELINE_SKIP_INTERACTIVE === "1") {
		const skipped = mergedConfig.sources.filter(
			(s) => EXTRACTOR_SOURCE_METADATA[s]?.requiresInteractiveChallenge,
		);
		if (skipped.length > 0) {
			console.log(
				"[headless] Skipping interactive sources: " + skipped.join(", "),
			);
			mergedConfig.sources = mergedConfig.sources.filter(
				(s) => !EXTRACTOR_SOURCE_METADATA[s]?.requiresInteractiveChallenge,
			);
		}
	}
		...
		try {
			...
			if (pendingChallenges.length > 0) {
				...
				try {
					...
					// Don't abort the whole pipeline — continue with jobs already
					// discovered in the first pass from the other sources.
					const message =
						retryError instanceof Error ? retryError.message : String(retryError);
					pipelineLogger.warn(
						"Challenge retry failed — continuing with results from other sources",
						{ sources: challengedSources, error: message },
					);
					sourceErrors = [
						...sourceErrors,
						...challengedSources.map((s) => `${s} (retry): ${message}`),
					];
					pendingChallenges = [];
				}
			...
			const perSource: Record<string, { jobsDiscovered: number; errors: number }> = {};
			for (const job of discoveredJobs) {
				const s = perSource[job.source] ?? { jobsDiscovered: 0, errors: 0 };
				s.jobsDiscovered++;
				perSource[job.source] = s;
			}
			...
			try {
				const watchlistMatches = await checkWatchlistMatches(importStartIso);
				if (watchlistMatches > 0) {
					pipelineLogger.info("Watchlist matches found", { watchlistMatches });
				}
			} catch (err) {
				pipelineLogger.warn("Watchlist check failed (non-fatal)", { err });
			}
...
export async function summarizeJob(
	...
	return runWithRequestContext({ jobId }, async () => {
		...
		try {
			...
			// OE toolkit: deterministic signals computed from job row
			let oeFitnessScore: number | undefined;
			let oeFitnessReasons: string | undefined;
			let redFlagsJson: string | undefined;
			let asyncScoreVal: number | undefined;
			...
			await jobsRepo.updateJob(job.id, {
				...
				asyncScore: asyncScoreVal,
				asyncSignals: asyncSignalsJson,
				weeklyHoursEstimate,
				weeklyHoursReasons,
			});
