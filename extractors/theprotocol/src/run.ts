...
export type TheprotocolProgressEvent =
...
export interface RunTheprotocolOptions {
  searchTerms?: string[];
  workplaceTypes?: Array<"remote" | "hybrid" | "onsite">;
  maxJobsPerTerm?: number;
  onProgress?: (event: TheprotocolProgressEvent) => void;
}

export interface TheprotocolResult {
  success: boolean;
  jobs: CreateJobInput[];
  error?: string;
...
function parseProgressLine(line: string): TheprotocolProgressEvent | null {
...
export async function runTheprotocol(options: RunTheprotocolOptions = {}): Promise<TheprotocolResult> {
  const useNpmCommand = canRunNpmCommand();
  if (!useNpmCommand && !TSX_CLI_PATH) {
    return { success: false, jobs: [], error: "Unable to execute theprotocol.it extractor (npm/tsx unavailable)" };
  }
  ...
  try {
    ...
    await new Promise<void>((resolve, reject) => {
      const extractorEnv = {
        ...
        THEPROTOCOL_SEARCH_TERMS: JSON.stringify(options.searchTerms ?? ["software engineer"]),
        THEPROTOCOL_WORKPLACE_TYPES: JSON.stringify(options.workplaceTypes ?? ["remote", "hybrid", "onsite"]),
        THEPROTOCOL_MAX_JOBS_PER_TERM: String(options.maxJobsPerTerm ?? 50),
        THEPROTOCOL_OUTPUT_JSON: DATASET_PATH,
      };
      ...
      child.on("close", (code) => {
        ...
        if (code === 0) resolve();
        else reject(new Error(`theprotocol.it extractor exited with code ${code}`));
      });
