...
import { runBulldogJob } from "./src/run";
...
function toProgress(event: {
  ...
  if (event.type === "term_start") {
    return {
      ...
      detail: `BulldogJob: term ${event.termIndex}/${event.termTotal} (${event.searchTerm})`,
    };
  }
  ...
  if (event.type === "page_fetched") {
    return {
      ...
      detail: `BulldogJob: term ${event.termIndex}/${event.termTotal}, page ${event.page ?? 0} (${event.totalCollected ?? 0} collected)`,
    };
  }
  ...
  return {
    ...
    detail: `BulldogJob: completed ${event.termIndex}/${event.termTotal} (${event.searchTerm}) — ${event.jobsFoundTerm ?? 0} jobs`,
  };
}
...
export const manifest: ExtractorManifest = {
  ...
  async run(context) {
    ...
    const result = await runBulldogJob({
