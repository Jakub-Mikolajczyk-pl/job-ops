...
function resolveLocation(group: RawGroup, offer: RawOffer): string {
	const displayWorkplace =
		typeof offer.displayWorkplace === "string" ? offer.displayWorkplace : "";
	const isRemote = group.isRemoteWorkAllowed === true;
	const workModes = Array.isArray(group.workModes)
		? (group.workModes as unknown[]).filter(
	...
	const hasRemote =
		...
		workModes.some(
			(m) =>
				m.toLowerCase().includes("remote") || m.toLowerCase().includes("home"),
		);
	const hasHybrid = workModes.some(
		(m) =>
			m.toLowerCase().includes("hybrid") ||
			m.toLowerCase().includes("mieszany"),
	);

	// Hybrid takes priority over remote when both flags are set (e.g. "1 day/week in office")
...
function mapGroup(group: RawGroup): CreateJobInput | null {
	const offers = Array.isArray(group.offers)
		? (group.offers as RawOffer[])
		: [];
	const firstOffer = offers[0];
	if (!firstOffer) return null;

	const offerUrl =
		typeof firstOffer.offerAbsoluteUri === "string"
			? firstOffer.offerAbsoluteUri
			: undefined;
	...
	const isRemote =
		!hasHybrid &&
		(group.isRemoteWorkAllowed === true ||
			workModes.some(
				(m) =>
					m.toLowerCase().includes("remote") ||
					m.toLowerCase().includes("home"),
			));
	...
	return {
		...
		sourceJobId,
		title:
			typeof group.jobTitle === "string" ? group.jobTitle : "Unknown Title",
		employer:
		...
		isRemote,
		datePosted:
			typeof group.lastPublicated === "string"
				? group.lastPublicated.slice(0, 10)
				: undefined,
		salary:
			typeof group.salaryDisplayText === "string"
...
async function extractJobData(
	pageHtml: string,
): Promise<{ groups: RawGroup[]; totalGroupCount: number } | null> {
	...
	const queries: unknown[] =
		(parsed as any)?.props?.pageProps?.dehydratedState?.queries ?? [];
	const jobQuery = (queries as any[]).find(
		(q: any) => Array.isArray(q?.queryKey) && q.queryKey[0] === "jobOffers",
	);
	if (!jobQuery) return null;

	const data = jobQuery.state?.data;
	const groups: RawGroup[] = Array.isArray(data?.groupedOffers)
		? data.groupedOffers
		: [];
...
async function run(): Promise<void> {
	const searchTerms = parseSearchTerms(
		process.env.PRACUJPL_SEARCH_TERMS,
		DEFAULT_SEARCH_TERM,
	);
	const maxJobsPerTerm = parsePositiveInt(
		process.env.PRACUJPL_MAX_JOBS_PER_TERM,
		DEFAULT_MAX_JOBS_PER_TERM,
	);
	const outputPath =
		process.env.PRACUJPL_OUTPUT_JSON ||
	...
	const workplaceTypes = parseWorkplaceTypes(
		process.env.PRACUJPL_WORKPLACE_TYPES,
	);
	...
	try {
		...
		try {
			const { challengeResult } = await navigateWithRetry(
				warmupPage,
				BASE_URL,
				{
					maxAttempts: 1,
			...
			while (termCollected < maxJobsPerTerm) {
				const maxPages =
					totalGroupCount !== null
						? Math.ceil(totalGroupCount / PAGE_SIZE)
						: 999;
				...
				if (totalGroupCount === null)
					totalGroupCount = extracted.totalGroupCount;
				...
				for (const group of extracted.groups) {
					...
					if (
						...
						if (!workplaceTypes.includes(wt)) {
							...
							if (!(wt === "hybrid" && workplaceTypes.includes("hybrid")))
								continue;
						}
					}
